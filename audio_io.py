...
class VADListener:
    def __init__(self, sample_rate=16000, frame_ms=30, vad_level=2, min_utt_ms=500, max_utt_ms=8000, hang_ms=600,
                 device=None, threshold=0.01):
        self.sr = sample_rate;
        self.frame_len = int(sample_rate * frame_ms / 1000)
        self.min_frames = int(min_utt_ms / frame_ms);
        self.max_frames = int(max_utt_ms / frame_ms);
        self.hang_frames = int(hang_ms / frame_ms)
        self.vad = webrtcvad.Vad(vad_level);
        self.device = device;
        self.threshold = threshold
        self.paused = False  # Add pause flag

    def listen(self, echo_guard=None, pause_check=None):
        with sd.RawInputStream(samplerate=self.sr, blocksize=self.frame_len, dtype='int16', channels=1,
                               device=self.device) as stream:
            voiced = 0;
            hang = 0;
            utt = []
            while True:
                # Check if paused (dictation mode)
                if pause_check and pause_check():
                    import time
                    time.sleep(0.1)
                    voiced = 0;
                    hang = 0;
                    utt = []  # Clear any partial utterance
                    continue
